...
for i=1:length(rel_pos(:,1))
    ...
    while rad_to_servo(q(1))<-0.1 || rad_to_servo(q(2))<-0.1||rad_to_servo(q(3))<-0.1 ||rad_to_servo(q(1))>1.1|| rad_to_servo(q(2))>1.1|| rad_to_servo(q(3))>1.1
        q = startq +([rand(), rand(), rand()]'-0.5)/1000;
        for j=1:5
            q = q + pinv(J(q, l)) * (x_target - endpos(q, l));
        end
